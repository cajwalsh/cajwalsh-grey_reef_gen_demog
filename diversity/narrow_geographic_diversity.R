#####==---- Narrower-scale geographic analysis of genetic diversity ----==##### -
...
diversity_pop_coords <- diversity_pop_coords[-c(1,3),]
...
xcoords <- rep(seq(xwest+0.125,xeast-0.125, 0.25), length(seq(ynorth-0.125, ysouth+0.125, -0.25)))
xcoords <- xcoords[order(xcoords)]
ycoords <- rep(seq(ynorth-0.125, ysouth+0.125, -0.25), length(seq(xwest+0.125,xeast-0.125, 0.25)))
...
# points(sp_coords, col = "deep pink 3", cex = 0.05, pch = 21)
...
Sys.time() - start_time # took 8 hours 51 minutes

narrow_r_div_coords <- cbind(filt_coords, unlist(r_list))

## Save this matrix as Rdata
save(narrow_r_div_coords, file = "narrow_r_div_coords.Rdata")

### Check plotting (load just results in to do it)
# load("narrow_r_div_coords.Rdata")
...
r_df <- merge(all_coords, narrow_r_div_coords,
              by = c("x", "y"), all = TRUE)
colnames(r_df) <- c("x", "y", "rho")

cor(diversity_pop_coords$pi_per_site, diversity_pop_coords$x, method = "spearman")

## Plot and save non-projected raster results map ## ----
library(ggplot2)
library(ggnewscale)
library(scales)
narrow_div_map <- ggplot() +
  ...
  geom_point(data = loc[which(loc$x > 100),],
             aes(x = x, y = y),  size = 2,
             shape = 3, stroke = 3/4) +
...
ggsave("narrow_div_map.pdf",
       plot = narrow_div_map, device = "pdf",
       width = 6.5,
       height = 6.5/abs(diff(c(xwest, xeast))/diff(c(ysouth, ynorth))),
       units = "in")

ggsave("narrow_div_map.tif",
       plot = narrow_div_map, device = "tiff",
       dpi = 800,
       width = 6.5,
       height = 6.5/abs(diff(c(xwest, xeast))/diff(c(ysouth, ynorth))),
       units = "in")
